...
const convertToMilliSeconds = (min, second) =>
  (Number(min) * 60 + Number(second)) * 1000;
...
export const padInputField = (inputField) => {
  ...
  } else inputField.value = padNumber(Number(inputField.value));
};
...
export const startTimer = () => {
  ...
  let timerInterval = setInterval(() => {
    ...
    let remainingMin = padNumber(
      Math.floor(Math.ceil(remainingTime / 1000) / SECONDS_IN_A_MINUTE)
    );
